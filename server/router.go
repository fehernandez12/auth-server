...
func (s *Server) router() http.Handler {
	...
	adminRouter.HandleFunc("/user/", s.HandleUser).Methods(http.MethodPost, http.MethodPatch)
	// adminRouter.HandleFunc("/user/roles", s.HandleUserRoles).Methods(http.MethodGet, http.MethodPost, http.MethodPatch)
	adminRouter.HandleFunc("/role/", s.HandleRole).Methods(http.MethodPost, http.MethodPatch)
	// adminRouter.HandleFunc("/role/permissions", s.HandleRolePermissions).Methods(http.MethodGet, http.MethodPost, http.MethodPatch)
	adminRouter.HandleFunc("/permission/", s.HandlePermission).Methods(http.MethodPost, http.MethodGet)
